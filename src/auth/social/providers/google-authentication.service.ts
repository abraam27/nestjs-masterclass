...
export class GoogleAuthenticationService implements OnModuleInit {
    ...
    public async authenticate(googleTokenDto: GoogleTokenDto) {
        ...
        if (!email || !googleId) {
            throw new UnauthorizedException('Invalid Google Token', { description: 'Invalid Google Token' });
        }
        ...
        return this.generateTokensProvider.generateTokens(createdUser);
            ...
            throw new UnauthorizedException(error, { description: 'Could not authenticate user' });
        }
    }
}
